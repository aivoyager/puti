...
from puti.conf.llm_config import LLMConfig, LlamaConfig
import random
...
async def test_generate_question():
    ...
    for index, row in tqdm(df.iterrows(), total=len(df), mininterval=0.5,
...
async def analyze_topic(text: str, node: OllamaNode) -> str:
    ...
    response = await node.chat(
        msg=[
            {"role": "system",
             "content": "你是一个专业的内容分析师，请分析以下文本并提取2-5个核心主题。对于每个主题，请提供中文和英文版本，多个之间换行区分，格式：中文主题/英文主题, 不需要其他多余的内容。特别注意处理中英文混合内容，确保双语主题的准确性和一致性。"},
...
async def generate_prompts(theme: str, node: OllamaNode) -> list:
    ...
    response = await node.chat(
        messages=[
            {"role": "system",
             "content": "请根据以下主题生成3条风格完全不同的推文指令。要求：1条提问式，1条感叹式，1条双语混合式。每条指令必须包含主题关键词，且风格差异明显。返回格式：指令1###指令2###指令3"},
...
async def is_question_reasonable(text: str, question: str, node: OllamaNode) -> bool:
    ...
    check_prompt = [
        {"role": "system",
         "content": "你是一个推文内容审核专家，请判断给定的推文正文（text）和对应的问题（question）是否匹配且合理。要求：1. question需与text主题相关，2. question应为推文生成指令或与推文内容高度相关。只需回答'合理'或'不合理'。"},
