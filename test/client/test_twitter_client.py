...
def test_data_convert_step2_length_strict():
    ...
    with open(str(root_dir() / 'data' / ''
        ...
        data = json.load(f)
    converted = [{"id": idx + 1, "text": item["response"]} for idx, item in enumerate(data) if
...
def test_post_tweet():
    t = TwikitClient()
    rs = t.cp.invoke(t.post_tweet, 'Hi, this is obstacles',
                     ['/Users/wangshuang/PycharmProjects/ws_algorithm/puti/puti/data/demo.png'], 1873694119337595083)
    ...
    try:
        with open(str(root_dir() / 'data' / 'trump.txt'), "a") as f:
            ...
            def save_recursion(twe):
                ...
                if twe.next_cursor:
                    tweet_next = asyncio.run(
                        t._cli.get_user_tweets('25073877', count=200, cursor=twe.next_cursor, tweet_type='Tweets'))
                    if tweet_next:
                        save_recursion(tweet_next)
