...
You have the following actions to choose from, please fully understand these actions, select the most appropriate action state
～～～
...
Your previous choose stage: {previous_state}
Based on history choose one of the following stages you need to go to in the next step (some stage has arguments)
No matter what, only return fixed JSON format {"state": a number between -1 ~ {n_states}, "arguments": {argument name if action have arguments: argument value...}}, don't reply anything else except json format
～～～
-1. Based on extra demands in system message if you have, if you think you have completed your goal, return {"state": -1, "arguments": {}} directly
{states}
～～～
